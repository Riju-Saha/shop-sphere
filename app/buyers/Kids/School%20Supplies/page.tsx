...
export default function SchoolSupplies() {
  ...
  const [schoolSuppliesProducts, setSchoolSuppliesProducts] = useState<any[]>([]);
  ...
  useEffect(() => {
    const category = 'Kids';
    const subCategory = 'SchoolSupplies';
    ...
    if (currentUsername && currentUsername !== 'Guest') {
      const fetchProducts = async () => {
        ...
        setSchoolSuppliesProducts(productsWithDate);
  ...
  const sortedSchoolSuppliesProducts = useMemo(() => {
    let productsCopy = [...schoolSuppliesProducts];
  ...
  }, [schoolSuppliesProducts, sortOption]);
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={styles.sectionsContainerStyles}>
          <h2 style={{ color: 'white', padding: '0 20px', marginBottom: '10px' }}>Inventory: {currentUsername}'s School Supplies ({sortedSchoolSuppliesProducts.length})</h2>

          {isLoadingProducts && <p style={{ color: 'gray', padding: '0 20px' }}>Loading products...</p>}

          {!isLoadingProducts && sortedSchoolSuppliesProducts.length === 0 && currentUsername !== 'Guest' && (
            <p style={{ color: 'gray', padding: '0 20px' }}>No School Supplies products found for this seller.</p>
          )}

          <div style={styles.productGrid}>
            {sortedSchoolSuppliesProducts.map((product, i) => (
              <div
                key={product.key || i}
                style={{
