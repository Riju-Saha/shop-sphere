...
export default function Tops() {
  ...
  const [topsProducts, setTopsProducts] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    if (currentUsername && currentUsername !== 'Guest') {
      const fetchProducts = async () => {
        ...
        setTopsProducts(productsWithDate);
  ...
  const sortedTopsProducts = useMemo(() => {
    let productsCopy = [...topsProducts];
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={styles.sectionsContainerStyles}>
          <h2 style={{ color: 'white', padding: '0 20px', marginBottom: '10px' }}>Inventory: {currentUsername}'s Tops ({sortedTopsProducts.length})</h2>

          {isLoadingProducts && <p style={{ color: 'gray', padding: '0 20px' }}>Loading products...</p>}

          {!isLoadingProducts && sortedTopsProducts.length === 0 && currentUsername !== 'Guest' && (
            <p style={{ color: 'gray', padding: '0 20px' }}>No Tops products found for this seller.</p>
          )}

          <div style={styles.productGrid}>
            {sortedTopsProducts.map((product, i) => (
              <div
                key={product.key || i}
                style={{
