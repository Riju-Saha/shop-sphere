...
export default function Laptops() {
  ...
  const [laptopsProducts, setLaptopsProducts] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    if (currentUsername && currentUsername !== 'Guest') {
      const fetchProducts = async () => {
        ...
        setLaptopsProducts(productsWithDate);
  ...
  const sortedLaptopsProducts = useMemo(() => {
    let productsCopy = [...laptopsProducts];
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={styles.sectionsContainerStyles}>
          <h2 style={{ color: 'white', padding: '0 20px', marginBottom: '10px' }}>Inventory: {currentUsername}'s Laptops ({sortedLaptopsProducts.length})</h2>
          ...
          <div style={styles.productGrid}>
            {sortedLaptopsProducts.map((product, i) => (
              <div
                key={product.key || i}
                style={{
