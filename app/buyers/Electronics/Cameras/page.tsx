...
export default function Cameras() {
  ...
  const [camerasProducts, setCamerasProducts] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    if (currentUsername && currentUsername !== 'Guest') {
      const fetchProducts = async () => {
        ...
        setCamerasProducts(productsWithDate);
  ...
  const sortedCamerasProducts = useMemo(() => {
    let productsCopy = [...camerasProducts];
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={styles.sectionsContainerStyles}>
          <h2 style={{ color: 'white', padding: '0 20px', marginBottom: '10px' }}>Inventory: {currentUsername}'s Cameras ({sortedCamerasProducts.length})</h2>

          {isLoadingProducts && <p style={{ color: 'gray', padding: '0 20px' }}>Loading products...</p>}

          {!isLoadingProducts && sortedCamerasProducts.length === 0 && currentUsername !== 'Guest' && (
            <p style={{ color: 'gray', padding: '0 20px' }}>No Cameras products found for this seller.</p>
          )}

          <div style={styles.productGrid}>
            {sortedCamerasProducts.map((product, i) => (
              <div
                key={product.key || i}
                style={{
