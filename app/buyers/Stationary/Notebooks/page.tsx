...
export default function Notebooks() {
  ...
  const [notebooksProducts, setNotebooksProducts] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    if (currentUsername && currentUsername !== 'Guest') {
      const fetchProducts = async () => {
        ...
        setNotebooksProducts(productsWithDate);
  ...
  const sortedNotebooksProducts = useMemo(() => {
    let productsCopy = [...notebooksProducts];
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={styles.sectionsContainerStyles}>
          <h2 style={{ color: 'white', padding: '0 20px', marginBottom: '10px' }}>Inventory: {currentUsername}'s Notebooks ({sortedNotebooksProducts.length})</h2>

          {isLoadingProducts && <p style={{ color: 'gray', padding: '0 20px' }}>Loading products...</p>}

          {!isLoadingProducts && sortedNotebooksProducts.length === 0 && currentUsername !== 'Guest' && (
            <p style={{ color: 'gray', padding: '0 20px' }}>No Notebooks products found for this seller.</p>
          )}

          <div style={styles.productGrid}>
            {sortedNotebooksProducts.map((product, i) => (
              <div
                key={product.key || i}
                style={{
