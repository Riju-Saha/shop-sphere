...
export default function Pens() {
  ...
  const [pensProducts, setPensProducts] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    if (currentUsername && currentUsername !== 'Guest') {
      const fetchProducts = async () => {
        ...
        setPensProducts(productsWithDate);
  ...
  const sortedPensProducts = useMemo(() => {
    let productsCopy = [...pensProducts];
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={styles.sectionsContainerStyles}>
          <h2 style={{ color: 'white', padding: '0 20px', marginBottom: '10px' }}>Inventory: {currentUsername}'s Pens ({sortedPensProducts.length})</h2>

          {isLoadingProducts && <p style={{ color: 'gray', padding: '0 20px' }}>Loading products...</p>}

          {!isLoadingProducts && sortedPensProducts.length === 0 && currentUsername !== 'Guest' && (
            <p style={{ color: 'gray', padding: '0 20px' }}>No Pens products found for this seller.</p>
          )}

          <div style={styles.productGrid}>
            {sortedPensProducts.map((product, i) => (
              <div
                key={product.key || i}
                style={{
