...
export default function TShirts() {
  ...
  useEffect(() => {
    ...
    if (currentUsername && currentUsername !== 'Guest') {
      const fetchProducts = async () => {
        ...
        setTShirtsProducts(productsWithDate);
  ...
  const sortedTShirtsProducts = useMemo(() => {
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={mainFlexWrapperStyles}>
        ...
        <div style={styles.sectionsContainerStyles}>
          <h2 style={{ color: 'white', padding: '0 20px', marginBottom: '10px' }}>Inventory: {currentUsername}'s TShirts ({sortedTShirtsProducts.length})</h2>

          {isLoadingProducts && <p style={{ color: 'gray', padding: '0 20px' }}>Loading products...</p>}

          {!isLoadingProducts && sortedTShirtsProducts.length === 0 && currentUsername !== 'Guest' && (
            <p style={{ color: 'gray', padding: '0 20px' }}>No TShirts products found for this seller.</p>
          )}

          <div style={styles.productGrid}>
            {sortedTShirtsProducts.map((product, i) => (
              <div
                key={product.key || i}
                style={{
